...
Deno.test('normalizeSource: defaults to whatsapp', () => {
  assertEquals(normalizeSource(undefined), 'whatsapp');
  assertEquals(normalizeSource('unknown'), 'whatsapp');
});

Deno.test('normalizeSource: accepts known values', () => {
  assertEquals(normalizeSource('whatsapp'), 'whatsapp');
