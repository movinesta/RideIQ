...
Deno.serve(withRequestContext(async (req) => {
  ...
  const hookSecret = envTrim('AUTH_HOOK_SEND_SMS_SECRET');
  if (hookSecret) {
    try {
    ...
    console.warn('[sms-hook] AUTH_HOOK_SEND_SMS_SECRET is not set; signature verification is disabled.');
  }
