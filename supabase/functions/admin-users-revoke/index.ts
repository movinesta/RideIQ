...
import { createAnonClient, requireUserStrict as requireUser } from '../_shared/supabase.ts';
...
Deno.serve((req) =>
  withRequestContext('admin-users-revoke', req, async (ctx) => {
    ...
    const { error: revErr } = await anon.rpc('admin_revoke_user', {
      p_user: body.user_id,
      p_note: note,
    });
    if (revErr) return errorJson(revErr.message, 400, 'DB_ERROR', undefined, ctx.headers);
