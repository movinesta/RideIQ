...
const PROMPT_INJECTION_PATTERNS: RegExp[] = [
  ...
  /OPENROUTER_API_KEY|SUPABASE_SERVICE_ROLE_KEY|SUPABASE_ANON_KEY/i,
  // Arabic variants
...
function containsSensitiveLeak(text: string): boolean {
  const t = text ?? "";
  if (/OPENROUTER_API_KEY|SUPABASE_SERVICE_ROLE_KEY|SUPABASE_ANON_KEY/i.test(t)) return true;
...
if (name === "merchant_sales_summary") {
  ...
  // For "sales" we exclude cancelled.
  const kept = orders.filter((o) => String(o?.status ?? "") !== "cancelled");
  const orderIds = kept.map((o) => String(o.id));
  ...
  const revenue = kept.reduce((s, o) => s + Number(o.total_iqd ?? 0), 0);
...
function pickForcedTool(surface: Surface, message: string): string | null {
  ...
  if (surface === "merchant" || surface === "merchant_chat" || surface === "driver") {
...
const MAX_TOOL_CALLS_PER_REQUEST = 6;
