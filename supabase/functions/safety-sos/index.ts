...
Deno.serve((req) => withRequestContext('safety-sos', req, async (ctx) => {
  ...
  try {
    ...
    if (!rideId) return errorJson('Missing ride_id', 400, 'VALIDATION_ERROR');
    ...
    const { data: sos, error: sosErr } = await service
      ...
      .select('id')
      .single();
    if (sosErr) return errorJson(sosErr.message, 500, 'DB_ERROR');
    ...
    if (autoNotify && (contacts ?? []).length) {
      ...
      // Outbox for provider-agnostic dispatch (WhatsApp/SMS/etc.).
