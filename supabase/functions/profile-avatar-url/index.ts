...
Deno.serve(async (req) => {
  ...
  if (body.action === 'upload') {
    ...
    const { data, error } = await anon.storage.from('avatars').createSignedUploadUrl(objectKey);
    ...
    if (body.set_profile) {
      const { error: upErr } = await anon.from('profiles').update({ avatar_object_key: objectKey }).eq('id', user.id);
  ...
  if (body.action === 'download') {
    ...
    const { data, error } = await anon.storage.from('avatars').createSignedUrl(body.object_key, expiresIn);
