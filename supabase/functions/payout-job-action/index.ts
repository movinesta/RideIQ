...
Deno.serve(async (req) => {
  ...
  return await withRequestContext('payout-job-action', req, async (ctx) => {
    try {
      ...
      if (!isUuid(jobId) || !action) return errorJson('Bad request', 400, 'BAD_REQUEST');
      ...
      const { data: job, error: jobErr } = await service
        ...
        .eq('id', jobId)
        .single();
      if (jobErr) return errorJson(jobErr.message, 404, 'NOT_FOUND');
      ...
      if (action === 'cancel') {
        const { data: updated, error: upErr } = await service
          ...
          .select('*')
          .single();
        if (upErr) return errorJson(upErr.message, 400, 'UPDATE_FAILED');
      ...
      if (job.status !== 'sent' && job.status !== 'confirmed') {
        return errorJson('Job must be sent first (or confirmed)', 409, 'INVALID_STATE');
      }
