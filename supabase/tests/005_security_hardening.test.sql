...
-- Ensure pricing config exists so other routines that depend on it have a row.
...
select throws_ok(
  $$ select public.quote_breakdown_iqd(33.3152, 44.3661, 33.3000, 44.4000, 'standard') $$,
  '42883',
  'legacy quote_breakdown_iqd is removed (undefined_function)'
);
