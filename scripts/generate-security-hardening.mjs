 ...
 *  - supabase/migrations/20260206009900_p0_security_hardening.sql (RECOMMENDED)
 *    Kept in sync so applying migrations yields the same security posture.
 *
...
// Keep deny-by-default grants in sync for both "schema.sql" and migrations.
// This repo uses date-stamped migrations; the hardening migration is required.
...
function generate() {
  ...
  // Update migration if present (recommended)
...
function main() {
  ...
  if (next.migrationOut.exists) {
    ...
    console.log(`Skipped: ${path.relative(ROOT, MIGRATION_PATH)} (not present)`);
  }
}
