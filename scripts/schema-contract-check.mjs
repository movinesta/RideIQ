...
function main() {
  ...
  // RPC allowlist ↔ grants check (deny-by-default requires explicit grants).
  const allowlistPath = path.join(ROOT, 'config', 'security', 'rpc-allowlist.json');
  if (fs.existsSync(allowlistPath)) {
    const allow = JSON.parse(readFile(allowlistPath));
    const anonAllowed = new Set((allow.anon ?? []).map((x) => String(x).toLowerCase()));
    const authAllowed = new Set((allow.authenticated ?? []).map((x) => String(x).toLowerCase()));

    const grantRe = /GRANT\s+(?:ALL|EXECUTE)\s+ON\s+FUNCTION\s+public\.([a-zA-Z0-9_]+)\([^;]*\)\s+TO\s+(anon|authenticated)\s*;/gi;
    const granted = new Map(); // fn -> Set(role)
    for (const m of sql.matchAll(grantRe)) {
      const fn = String(m[1]).toLowerCase();
      const role = String(m[2]).toLowerCase();
      const set = granted.get(fn) ?? new Set();
      set.add(role);
      granted.set(fn, set);
    }

    const missing = [];
    for (const fn of anonAllowed) {
      if (!(granted.get(fn)?.has('anon'))) missing.push(`anon:${fn}`);
    }
    for (const fn of authAllowed) {
      if (!(granted.get(fn)?.has('authenticated'))) missing.push(`authenticated:${fn}`);
    }

    const extra = [];
    for (const [fn, roles] of granted.entries()) {
      if (roles.has('anon') && !anonAllowed.has(fn)) extra.push(`anon:${fn}`);
      if (roles.has('authenticated') && !authAllowed.has(fn)) extra.push(`authenticated:${fn}`);
    }

    if (missing.length) {
      errors.push('RPC allowlist functions missing GRANTs: ' + missing.join(', '));
    }
    if (extra.length) {
      errors.push('RPC GRANTs present but not allowlisted: ' + extra.join(', '));
    }
  } else {
    errors.push('Missing allowlist file: ' + path.relative(ROOT, allowlistPath));
  }
