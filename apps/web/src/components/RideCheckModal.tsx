...
export default function RideCheckModal({ open, event, rideId, onClose, onResolved }: Props) {
  ...
  const respond = async (response: 'ok' | 'false_alarm' | 'need_help') => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('ridecheck-respond', {
        body: { event_id: event.id, response },
      });
      ...
      if (response === 'need_help') {
        ...
        const { error: sosErr } = await supabase.functions.invoke('safety-sos', {
          body: { ride_id: rideId, lat: geo?.lat, lng: geo?.lng },
        });
        if (sosErr) {
