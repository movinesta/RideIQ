...
import { buildWhatsAppClickToChatUrl, buildWhatsAppShareUrl } from '../lib/whatsapp';
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    setToast(null);
                    try {
                      const { data } = await invokeEdge<TripShareCreateResponse>('trip-share-create', {
                        ride_id: rideId,
                  ...
                  <div className="mt-3 space-y-2">
                    {trustedContacts.map((c: any) => (
                      <div key={c.id} className="flex items-center justify-between gap-3 rounded-2xl border border-gray-200 p-3">
                        ...
                        <a
                          className="btn"
                          href={buildWhatsAppClickToChatUrl(c.phone, shareMessage)}
                          target="_blank"
                          rel="noreferrer"
                        >
                          {t('safety.share.toContact')}
                        </a>
                      </div>
                    ))}
                  </div>
