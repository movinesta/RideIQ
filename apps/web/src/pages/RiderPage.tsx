...
async function fetchRides(): Promise<RideRow[]> {
  const { data, error } = await supabase
    .from('rides')
    .select('id,request_id,status,version,created_at,started_at,completed_at,fare_amount_iqd,currency,paid_at,pickup_pin_required,pickup_pin_verified_at')
...
const mapCenter = pickupPos ?? dropoffPos ?? { lat: 33.3152, lng: 44.3661 };
  ...
  const quote = useQuery({
    ...
    queryFn: async (): Promise<QuoteBreakdown | null> => {
      ...
      const { data, error } = await supabase.rpc('quote_breakdown_iqd', {
        p_pickup_lat: pl,
        p_pickup_lng: pg,
        p_dropoff_lat: dl,
        p_dropoff_lng: dg,
    ...
    {nearbyDrivers.isFetching ? ' (refreshing...)' : ''}
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <button
            ...
            disabled={busy}
            onClick={async () => {
              const number = getWhatsAppBookingNumber();
              if (!number) {
                setToast('Missing VITE_WHATSAPP_BOOKING_NUMBER.');
                return;
              }
              if (!serviceArea?.id) {
                setToast('Pickup is outside supported service areas.');
              ...
              setToast(null);
              try {
                const whenIso = mode === 'scheduled' && scheduledAtLocal ? new Date(scheduledAtLocal).toISOString() : null;
				type RideIntentCreateResponse = { intent?: { id?: string } };
				const { data: intentData } = await invokeEdge<RideIntentCreateResponse>('ride-intent-create', {
                  pickup_address: pickupAddress,
                  dropoff_address: dropoffAddress,
                  pickup_lat: Number(pickupLat),
                  pickup_lng: Number(pickupLng),
				...
				const intentId = intentData?.intent?.id ?? 'unknown';
                const lines = [
                  'RideIQ booking request',
                  `Intent: ${intentId}`,
                  whenIso ? `Scheduled: ${whenIso}` : 'Mode: Now',
                  serviceArea?.name ? `Service area: ${serviceArea.name}` : null,
                  pickupAddress ? `Pickup: ${pickupAddress}` : null,
                  dropoffAddress ? `Dropoff: ${dropoffAddress}` : null,
                  `Pickup coords: ${pickupLat}, ${pickupLng}`,
                  `Dropoff coords: ${dropoffLat}, ${dropoffLng}`,
                ].filter(Boolean);

                const url = buildWhatsAppClickToChatUrl(number, lines.join('\n'));
                window.open(url, '_blank', 'noopener,noreferrer');
                setToast('Opened WhatsApp. Please send the message to complete your request.');
              } catch (e: unknown) {
                setToast(`Error: ${errorText(e)}`);
              } finally {
                setBusy(false);
