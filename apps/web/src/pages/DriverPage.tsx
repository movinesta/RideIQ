...
async function fetchRides(): Promise<RideRow[]> {
  ...
  const { data, error } = await supabase
    .from('rides')
    .select(
      'id,request_id,status,version,created_at,started_at,completed_at,fare_amount_iqd,currency,pickup_pin_required,pickup_pin_verified_at,pickup_pin_fail_count,pickup_pin_locked_until',
    )
    .eq('driver_id', uid)
    .order('created_at', { ascending: false })
    .limit(20);
  ...
  const verifyPin = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('ride-verify-pin', {
        body: { ride_id: pinRide.id, pin: pinValue.trim() },
      });
      if (error) throw error;
  ...
  const transitionRide = async (ride: RideRow, to: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('ride-transition', {
        body: { ride_id: ride.id, to_status: to },
      });
      if (error) throw error;
