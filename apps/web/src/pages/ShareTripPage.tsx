...
async function fetchShare(token: string): Promise<ShareResponse> {
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    throw new Error('Missing VITE_SUPABASE_URL or VITE_SUPABASE_ANON_KEY (or alias VITE_SUPABASE_PUBLISHABLE_KEY)');
  }

  const url = `${SUPABASE_URL.replace(/\/$/, '')}/functions/v1/trip-share-view?token=${encodeURIComponent(token)}`;
